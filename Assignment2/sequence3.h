...
namespace main_savitch_5
{
    ...
    public:
        ...
        size_t size( ) const{ //O(n) with linear time
            return list_length(headptr);
        };
        bool is_item( ) const{ //O(1) with constant time
            if (currentptr == NULL){
                return false;
            }
            else{
        ...
        value_type current( ) const{ //O(1) with constant time
            if (is_item()){
