...
namespace main_savitch_10
{
    ...
    // of the target were removed.
    {
        ...
        /* STUDENT WORK */
        else{ //if root_ptr left isn't null, shift rest of left tree up to replace target
            binary_tree_node<Item> *max = root_ptr->left();
            root_ptr->data() = max->data();
            root_ptr->left() = max->left();
            root_ptr->right() = max->right();

            bst_remove_all(root_ptr, target); //search from rootptr in case this is also target
            return 1;
        }
    }
